...
function readOrgStructured(ss) {
  ...
  // ★ 2026.08.05 수정 — 정확일치를 부분일치보다 먼저 본다.
  //   기존 구현은 키워드별로 '정확일치 or 부분일치'를 한 번에 찾았기 때문에
  //   헤더 순서상 **부분일치가 정확일치를 앞질렀다**.
  //   실제 사고: 헤더가 [... '본부/부서'(3), '부서'(4) ...] 인데
  //     · fi('본부') → 3 (의도대로)
  //     · fi('부서') → **3** ('본부/부서'가 '부서'를 포함하고 더 앞이라서)
  //   → 본부부서 === 본부 가 되어 실제 '부서' 컬럼(AM 부서·재무 부서)이
  //     한 번도 읽히지 않았다. 그 결과 대시보드 조직도에서 **부서 레벨이 통째로
  //     사라지고**(renderOrg의 deptMap이 단일 키가 되어 '직속'으로 처리됨),
  //     조직도 스냅샷(6-B)의 '부서' 컬럼도 본부 값을 복사하고 있었다.
  //   ※ 이 변경으로 값이 달라지는 컬럼은 **본부부서 하나뿐**이다 (오프라인 대조 완료).
  //     나머지 12개 컬럼은 정확일치 헤더가 없거나 이미 정확일치가 먼저 걸려 동일.
  const fi = (...kws) => {
    for (const kw of kws) {
      let idx = hdr.findIndex(h => h === kw);           // ① 정확일치 우선
      if (idx >= 0) return idx;
      idx = hdr.findIndex(h => h.includes(kw));         // ② 부분일치 폴백
      if (idx >= 0) return idx;
    }
    return -1;
  };
